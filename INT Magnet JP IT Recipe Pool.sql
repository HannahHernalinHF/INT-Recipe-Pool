...
where country IN ('IT','JP') and score>0 and rating_count>0
) t where o=1
group by 1,2
)
...
-------
,seasonality as(
select sku,max(seasonality_score) as seasonality_score from uploads.gp_sku_seasonality where country IN ('JP','IT') and week>='W37'and week<='W65'
group by 1
)
, recipe_usage as(
select * from materialized_views.isa_services_recipe_usage
where region_code in ('jp','it') and market in ('jp','it')
)

, nutrition as(
select *
from materialized_views.culinary_services_recipe_segment_nutrition
where segment in ('JP','IT') and market in ('jp','it')
)

,cost as(
select recipe_id, size, avg(price) as cost
from materialized_views.culinary_services_recipe_static_price
where segment IN ('IT','JP')  and hellofresh_week >= '2022-W37' and hellofresh_week <= '2022-W65'
group by 1,2
)
...
where  sku.market in ('jp','it') and sp.hellofresh_week >= '2022-W37' and sp.hellofresh_week <= '2022-W65'
group by 1
)
...
,picklists as(
    ...
    from (
        ...
        where r.market in ('jp','it') and p.segment_name IN ('JP','IT')
