...
where country='JP' and score>0 and rating_count>0
) t where o=1
group by 1,2
)
...
-------
,seasonality as(
select sku,max(seasonality_score) as seasonality_score from uploads.gp_sku_seasonality where country='JP' --and week>='W31'and week<='W34'
group by 1
)
, recipe_usage as(
select * from materialized_views.isa_services_recipe_usage
where region_code='jp' and market = 'jp'
)

, nutrition as(
select *
from materialized_views.culinary_services_recipe_segment_nutrition
where segment = 'JP' and market = 'jp'
)

,cost as(
select recipe_id, size, avg(price) as cost
from materialized_views.culinary_services_recipe_static_price
where segment='JP' --and hellofresh_week >= '2022-W37' and hellofresh_week <= '2022-W40'
group by 1,2
)
...
where  sku.market='jp' --and sp.hellofresh_week >= '2022-W37' and sp.hellofresh_week <= '2022-W40'
group by 1
)
...
where r.market = 'jp' and p.segment_name = 'JP'
